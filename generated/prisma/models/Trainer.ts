...
export interface TrainerDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
  ...
  /**
   * Find zero or more Trainers that matches the filter.
   * @param {TrainerFindRawArgs} args - Select which filters you would like to apply.
   * @example
   * const trainer = await prisma.trainer.findRaw({
   *   filter: { age: { $gt: 25 } }
   * })
   */
  ...
  /**
   * Perform aggregation operations on a Trainer.
   * @param {TrainerAggregateRawArgs} args - Select which aggregations you would like to apply.
   * @example
   * const trainer = await prisma.trainer.aggregateRaw({
   *   pipeline: [
